...
async function seedCompleteDatabase() {
  ...
  // 3. Clean and seed Categories & Subcategories
  ...
  for (const catData of CATEGORIES_DATA) {
    ...
    let cat = await db.collection('categories').findOne({ name: catData.name, parentId: null });
  ...
  for (let idx = 0; idx < PRODUCTS_DATA.length; idx++) {
    ...
    const sku = `SKU-${p.productName.substring(0, 4).toUpperCase().replace(/[^A-Z]/g, 'X')}-${idx + 100}`;
    const slug = p.productName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
