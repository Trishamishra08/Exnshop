...
async function seedCompleteCatalog() {
  ...
  for (const def of sectionDefs) {
    ...
    let cat = await db.collection('categories').findOne({ slug: def.slug });
    if (!cat) {
      cat = await db.collection('categories').findOne({ name: def.title });
    }
    if (!cat) {
    ...
    for (const subDef of def.subcategories) {
      ...
      if (!subDoc) {
        ...
        await db.collection('categories').updateOne(
          { _id: subDoc._id },
          { $set: { image: subDef.image, status: 'Active' } }
        );
      }
    ...
    console.log(`✅ ${def.title}: Created & linked ${subIds.length} subcategories to HomeSection`);
  }

  await mongoose.disconnect();
  console.log('\n🎉 ALL 4 SECTIONS & SUBCATEGORIES POPULATED SUCCESSFULLY!');
}
