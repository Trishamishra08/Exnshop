...
async function seedExact4Sections() {
  ...
  for (const def of sectionDefs) {
    ...
    for (const subDef of def.subcategories) {
      ...
      if (!subDoc) {
        ...
        await db.collection('categories').updateOne({ _id: subDoc._id }, { $set: { status: 'Active', image: subDef.image } });
        subIds.push(subDoc._id);
      }
    }

    // Upsert HomeSection document
  ...
  console.log('\n🎉 ALL SECTIONS SEEDED PERFECTLY!');
}

seedExact4Sections().catch(console.error);
