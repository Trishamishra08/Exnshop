...
const SHOPS_DATA = [
  {
    storeId: "supermarket-essentials",
    name: "Olovely Supermart",
    image: "/assets/shopbystore/fashion.jpg",
    order: 1
  },
  {
    storeId: "dairy-farm-fresh",
    name: "Daily Dairy & Bakery",
    image: "/assets/shopbystore/pet.jpg",
    order: 2
  },
  {
    storeId: "snack-refreshment",
    name: "Snack & Munchies Hub",
    image: "/assets/shopbystore/sports.jpg",
    order: 3
  },
  {
    storeId: "organic-green",
    name: "Farm Fresh Organic",
    image: "/assets/shopbystore/pharma.jpg",
    order: 4
  },
  {
    storeId: "home-cleaning",
    name: "Clean Home & Care",
    image: "/assets/shopbystore/toy.jpg",
    order: 5
  },
  {
    storeId: "spiritual-store",
    name: "Pooja & Spiritual Needs",
    image: "/assets/shopbystore/spiritual.jpg",
    order: 6
  }
];
...
async function seedFullCatalog() {
  ...
  console.log('✅ Approved Seller active:', seller._id);

  // 2. Header Categories
  const headerMap: Record<string, mongoose.Types.ObjectId> = {};
  for (const h of HEADER_CATEGORIES) {
    let header = await HeaderCategory.findOne({ slug: h.slug });
    if (header) {
      Object.assign(header, h);
      await header.save();
    } else {
      header = await HeaderCategory.create(h);
    }
    headerMap[h.slug] = header._id as mongoose.Types.ObjectId;
  }
  console.log(`✅ Header Categories seeded: ${Object.keys(headerMap).length}`);

  // 3. Main Categories & Subcategories
  ...
  for (const catData of CATEGORIES_DATA) {
    ...
    if (cat) {
      Object.assign(cat, catDoc);
      await cat.save();
    } else {
      cat = await Category.create(catDoc);
    }
    categoryMap[catData.name] = cat._id as mongoose.Types.ObjectId;
    ...
    if (catData.subcategories && catData.subcategories.length > 0) {
      for (let sIdx = 0; sIdx < catData.subcategories.length; sIdx++) {
        ...
        let sub = await Category.findOne({ name: subName, parentId: cat._id });
        ...
        if (sub) {
          Object.assign(sub, subDoc);
          await sub.save();
        } else {
          sub = await Category.create(subDoc);
        }
        subcategoryMap[`${catData.name}__${subName}`] = sub._id as mongoose.Types.ObjectId;
      }
    }
  }
  ...
  // 4. Products Seeding
  const allCreatedProductIds: mongoose.Types.ObjectId[] = [];
  ...
  for (const p of PRODUCTS_DATA) {
    ...
    let prod = await Product.findOne({ productName: p.productName });
    if (prod) {
      Object.assign(prod, productDoc);
      await prod.save();
      allCreatedProductIds.push(prod._id as mongoose.Types.ObjectId);
    } else {
      prod = await Product.create(productDoc);
      allCreatedProductIds.push(prod._id as mongoose.Types.ObjectId);
    }
  }
  console.log(`✅ Products seeded: ${allCreatedProductIds.length}`);

  // 5. Shops / Stores Seeding
  await Shop.deleteMany({});
  for (let idx = 0; idx < SHOPS_DATA.length; idx++) {
    const s = SHOPS_DATA[idx];
    const shopProducts = allCreatedProductIds.slice(idx * 4, (idx + 1) * 4);
    await Shop.create({
      storeId: s.storeId,
      name: s.name,
      image: s.image,
  ...
  console.log(`✅ Shops seeded: ${SHOPS_DATA.length}`);

  // 6. Lowest Prices Ever Section
  await LowestPricesProduct.deleteMany({});
  for (let idx = 0; idx < Math.min(8, allCreatedProductIds.length); idx++) {
    await LowestPricesProduct.create({
      product: allCreatedProductIds[idx],
      order: idx + 1,
      isActive: true,
    });
  }
  console.log(`✅ LowestPricesProducts populated`);

  // 7. Bestseller Cards (Top categories featuring real products)
  ...
  for (let idx = 0; idx < bestsellerCategoryNames.length; idx++) {
    const cName = bestsellerCategoryNames[idx];
    const catId = categoryMap[cName];
    if (catId) {
  ...
  console.log(`✅ BestsellerCards populated`);

  // 8. Promo Strip (Housefull / Super Saver Deals for Home Page)
  await PromoStrip.deleteMany({});
  const featuredProds = allCreatedProductIds.slice(0, 6);
  const promoCatCards = [
  ...
  console.log('✅ PromoStrip created for header "all"');

  console.log('\n🎉 ALL CATALOG DATA SEEDED SUCCESSFULLY FOR CLIENT UI REVIEW!');
