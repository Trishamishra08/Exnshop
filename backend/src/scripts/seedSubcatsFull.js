...
async function seedSubcatsFull() {
  ...
  for (const def of sectionDefs) {
    let cat = await db.collection('categories').findOne({ name: def.title });
    if (!cat) {
      ...
      cat = await db.collection('categories').findOne({ _id: res.insertedId });
    }
    ...
    for (const subDef of def.subcategories) {
      ...
      let subDoc = await db.collection('categories').findOne({ parentId: cat._id, name: subDef.name });
      if (!subDoc) {
        const res = await db.collection('categories').insertOne({
          name: subDef.name,
          slug: subSlug,
        ...
        subIds.push(res.insertedId);
      } else {
        // Update image if needed
        await db.collection('categories').updateOne({ _id: subDoc._id }, { $set: { image: subDef.image, status: 'Active' } });
    ...
    console.log(`✅ ${def.title}: Linked ${subIds.length} subcategories to HomeSection`);
  }
...
seedSubcatsFull().catch(console.error);
