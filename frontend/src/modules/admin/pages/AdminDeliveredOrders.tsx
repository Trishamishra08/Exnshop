...
export default function AdminDeliveredOrders() {
  ...
  const handleExport = () => {
    const headers = ['Order ID', 'Customer Name', 'Phone', 'Address', 'Order Date', 'Delivery Date', 'Status', 'Delivery Status', 'Total Amount'];
    const csvContent = [
      headers.join(','),
      ...
      filtered.sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case 'address':
            ...
            break;
          case 'deliveryDate':
            aValue = a.deliveredAt || a.estimatedDeliveryDate || '';
            bValue = b.deliveredAt || b.estimatedDeliveryDate || '';
            break;
          case 'orderDate':
  ...
  const stats = useMemo(() => {
    ...
    return { totalCount, totalRevenue, avgOrderValue };
  }, [orders]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-neutral-200/80 shadow-sm">
        <div>
          <div className="flex items-center gap-2 text-xs text-neutral-500 mb-1">
            ...
            <span>/</span>
            <span className="text-emerald-600 font-medium">Delivered Orders</span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={() => {
              setCurrentPage(1);
              setLoading(true);
              getOrdersByStatus('Delivered', { page: 1, limit: parseInt(entriesPerPage), channel: mode })
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-neutral-200/80 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-xl font-extrabold text-neutral-900">₹{stats.avgOrderValue.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</h3>
          </div>
        </div>
      </div>
            ...
            <thead>
              <tr className="bg-neutral-100/60 border-b border-neutral-200 text-[11px] font-bold text-neutral-500 uppercase tracking-wider">
                ...
                >
                  <div className="flex items-center gap-1">
                    Delivery Date
                    {sortField === 'deliveryDate' && (<span>{sortDirection === 'asc' ? '↑' : '↓'}</span>)}
                  </div>
                </th>
                ...
                <tr>
                  <td colSpan={9} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-sm font-medium text-neutral-500">Loading delivered orders...</p>
                    </div>
                  </td>
                </tr>
              ) : error ? (
                ...
                <tr>
                  <td colSpan={9} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center gap-2 text-neutral-400">
                      ...
                      <p className="text-base font-semibold text-neutral-700">No Delivered Orders Found</p>
                      <p className="text-xs text-neutral-500">No delivered orders match your active filter parameters.</p>
                    ...
                    <tr key={order._id} className="hover:bg-neutral-50/80 transition-colors group">
                      ...
                      <td className="px-4 py-3.5 text-neutral-600 whitespace-nowrap font-medium">
                        {order.deliveredAt || order.estimatedDeliveryDate
                          ? new Date(order.deliveredAt || order.estimatedDeliveryDate!).toLocaleDateString('en-IN', {
                              day: '2-digit',
                              month: 'short',
                              year: 'numeric'
                            })
